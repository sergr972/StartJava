...
//        }
        ...
        int sum = 567;
        int hundredDollars = 4;
        int tenDollars = 5;
        int oneDollars = 50;
        int hundreds = sum / 100;
        int ten = sum % 100 / 10;
        int ones = sum % 10;
        int numHundredDollars = 0, numTenDollars = 0, numOneDollars = 0;

        if (hundredDollars >= hundreds) {
            numHundredDollars = hundreds;
        } else {
            numHundredDollars = hundredDollars;
            numTenDollars = (hundreds - numHundredDollars) * 10;
        }
        if ((tenDollars - numTenDollars) >= ten) {
            numTenDollars += ten;
        } else {
            numTenDollars = tenDollars;
            numOneDollars = (ten - numTenDollars) * 10;

        if ((oneDollars - numOneDollars) >= ones) {
            numOneDollars += ones;
        } else {
            numOneDollars += oneDollars;

        }
            System.out.println("банкнот номиналом 100 - " + numHundredDollars);
            System.out.println("банкнот номиналом 10 - " + numTenDollars);
            System.out.println("банкнот номиналом 1 - " + numOneDollars);
            System.out.println("исходная сумма - " + (numHundredDollars * 100 + numTenDollars * 10 + numOneDollars * 1));
